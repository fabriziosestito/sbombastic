...
func TestTraceContextHandler_NoSpan(t *testing.T) {
	...
	var record map[string]any
	require.NoError(t, json.Unmarshal(buf.Bytes(), &record))
	_, hasTrace := record[traceIDKey]
	_, hasSpan := record[spanIDKey]
	assert.False(t, hasTrace, "trace_id should be absent when no span")
	assert.False(t, hasSpan, "span_id should be absent when no span")
}
